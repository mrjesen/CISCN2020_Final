...
if (isset($_FILES["file"])) {
    ...
    )
    {
        ...
        }  else if (preg_match("/php|HALT|flag/i", file_get_contents($_FILES["file"]["tmp_name"]) )){
            $user->alertMes("dangerous file content", "./dashboard.php");
        }else {
            $tmp = time();
            move_uploaded_file($_FILES["file"]["tmp_name"], "upload/" . MD5($tmp) . ".png.tmp");
            $user->set_avatar("upload/" . MD5($tmp) . ".png.tmp");
            echo "upload/" . MD5($tmp) . ".png.tmp";
        }
    } else {
